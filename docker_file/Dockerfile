...
RUN mkdir -p /usr/portage/gentoo && emerge-webrsync && emerge --sync && \
    eselect profile set 5 && emerge app-crypt/gnupg \
    ...
    echo "ros-melodic/* PYTHON_TARGETS: -* python2_7" >> /etc/portage/package.use/iputils && \
    echo "ros-melodic/* PYTHON_SINGLE_TARGET: -* python2_7" >> /etc/portage/package.use/iputils && \
    ...
    dev-qt/qtxml app-portage/repoman \
